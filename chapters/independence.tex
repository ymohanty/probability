...
It captures the degree to which a random variable deviates from its
mean value. The third \emph{standardized }moment $\mathbb{E}\left[\left(\frac{Y}{\sigma}\right)^{3}\right]$
...
}of $X$. Of course, random variables need not even by $\mathcal{L}^{1}$,
...
symmetric about $0$ if for any $x\in\R$$:F_{X}(x)=1-F_{X}\left(-x\right).$} and in the fourth the fact that the derivative of $\tan^{-1}\left(x\right)$
...
a product measure, we can use Tonelli/Fubini's theorem to turn it
into an iterated integral. The final equality is yet another application
